    ...
    try {
      ...
      if (res.data && res.data.token) {
        ...
        if (res.data.user && res.data.user.createdAt) {
          ...
          isNewUser = diffMinutes < 5; // User created within last 5 minutes
          console.log("User created:", createdAt, "Minutes ago:", diffMinutes, "Is new:", isNewUser);
        }
        
        // Check hasVisited flag

const isNewSignup = location.state?.isNewSignup;
const hasVisited = localStorage.getItem("hasVisited");

if (isNewSignup && hasVisited !== "true") {
  navigate("/welcome", { replace: true, state: {} });  // state clear
} else {
  navigate("/dashboard", { replace: true, state: {} }); // state clear
}
